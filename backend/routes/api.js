...
function listHomeReceipts(_req, res) {
  res.json({
    description:
      "Receipt images for scanning from Home / gallery (src/assets/demo-receipts).",
    receipts: receiptSimulator.RECEIPT_CATALOG.map(
      receiptSimulator.formatReceipt,
    ),
  });
}
...
function listPayQrMerchantsFull(_req, res) {
  ...
  if (!fs.existsSync(PAY_QR_MERCHANTS)) {
    return res.json({
      description: "Run npm run generate:assets",
      merchants: [],
    });
  }
  ...
  res.json({
    description:
      "Merchant pay QR codes (src/assets/demo-qr). Scan via Pay → QR Code.",
...
router.post(
  "/users/:userId/transactions/receipt",
    ...
    if (receipt.cardNumber) {
      ...
      if (!card) {
        return res.status(400).json({
          error:
            "This receipt was paid with a card that is not linked to your account.",
        });
      }
    } else {
      card = await db.resolveCardForPayment(
        req.params.userId,
      ...
      if (!card) {
        return res
          .status(400)
          .json({ error: "No linked card found for this payment method." });
      }
    }

    const saved = await db.addTransaction(
      createTransactionFromReceipt(req.params.userId, receipt, card),
    );
  ...
  asyncHandler(async (req, res) => {
    ...
    if (result.kind === "receive") {
      ...
      if (!recipient) {
        return res
          .status(400)
          .json({ error: "Recipient in this QR is not registered." });
      }

      const receiveTarget = cardUtils.resolveReceiveCard(
        await db.getCards(recipient.id),
      );
      return res.json({
        success: true,
        kind: "receive",
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ error: "No NETS user found for this mobile number." });
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!parsed.ok || parsed.kind !== "pay" || !parsed.payment) {
      return res
        .status(400)
        .json({ error: parsed.error || "Invalid payment QR." });
    }
    ...
    if (!card) {
      return res
        .status(400)
        .json({ error: "No linked card found for this payment." });
    }
    ...
    if (!spendingRules.canSpendOnCard(card, spendContext)) {
      return res
        .status(400)
        .json({ error: spendingRules.spendBlockMessage(card, spendContext) });
    }
    ...
    if (req.body.voucherInstanceId) {
      ...
      if (voucherPreview.ok) {
        ...
        paymentToCharge = {
          ...parsed.payment,
          amount: voucherPreview.finalAmount,
        };
      } else {
    ...
    if (!cardUtils.hasSufficientFunds(card, paymentToCharge.amount)) {
      return res
        .status(400)
        .json({ error: "Insufficient balance on the selected card." });
    }

    const saved = await db.addTransaction(
      qrPayment.createTransactionFromQr(
        req.params.userId,
        paymentToCharge,
        card,
      ),
    ...
    const refreshedCard = await db.setCardBalance(
      card.id,
      cardUtils.applyDebit(card, paymentToCharge.amount),
    );
    ...
    if (voucherApplied) {
      const commitResult = await myVouchers.commitVoucherUsage(
        req.params.userId,
        req.body.voucherInstanceId,
        {
    ...
    const voucherNote = voucherApplied
...
router.post(
  "/users/:userId/payments/qr/receive",
  asyncHandler(async (req, res) => {
    const user = await db.getUser(req.params.userId);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    ...
    if (!parsed.ok || parsed.kind !== "receive" || !parsed.receive?.userId) {
      return res
        .status(400)
        .json({ error: parsed.error || "Invalid receive QR." });
    }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const cards = (await db.getCards(req.params.userId, cardType)).map((row) =>
      mapCard(row, view),
    );
    res.json({ cards });
  }),
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const cards = (await db.getCards(req.params.userId)).map((row) =>
      mapCard(row, "wallet"),
    );
...
router.post(
  "/users/:userId/cards/generate-number",
  asyncHandler(async (req, res) => {
    const user = await db.getUser(req.params.userId);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    ...
    if (cardType !== "prepaid" && cardType !== "cashcard") {
      return res
        .status(400)
        .json({ error: "Only prepaid and CashCard can be issued." });
    }
    ...
    if (!digits) {
      return res
        .status(503)
        .json({ error: "Unable to generate a unique card number. Try again." });
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const digits = netsSimulator.normalizeDigits(req.body.cardNumber);
    const existingForUser = await db.findLinkedCardByNumber(
      req.params.userId,
      digits,
    );
    if (existingForUser) {
      return res
        .status(409)
        .json({ error: "This card is already linked to your account." });
    }
    ...
    if (linkedElsewhere && linkedElsewhere.user_id !== req.params.userId) {
      return res
        .status(409)
        .json({ error: "This card is already linked to another account." });
    }

    const userCards = await db.getCards(req.params.userId);
    const isOthersDebit =
      linkResult.card.cardType === "others" &&
      linkResult.card.accountKind === "debit";
    const hasDefaultDebit = userCards.some(
      (c) =>
        c.card_type === "others" &&
        c.account_kind === "debit" &&
        c.is_default_receive,
    );
    const shouldDefaultReceive =
      isOthersDebit && (linkResult.card.isDefaultReceive || !hasDefaultDebit);
...
router.patch(
  "/users/:userId/cards/:cardId/default-receive",
  asyncHandler(async (req, res) => {
    const user = await db.getUser(req.params.userId);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
...
router.patch(
  "/users/:userId/cards/:cardId/top-up-preference",
  asyncHandler(async (req, res) => {
    const user = await db.getUser(req.params.userId);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    ...
    const result = await db.setCardTopUpEnabled(
      req.params.userId,
      req.params.cardId,
      enabled,
    );
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await db.markNotificationRead(
      req.params.userId,
...
router.post(
  "/users/:userId/payogotchi/milestone-bonus",
  asyncHandler(async (req, res) => {
    const { fromLevel, toLevel, evolved, newStage } = req.body || {};
    const result = await payogotchiRewards.awardPetMilestone(
      req.params.userId,
      {
        fromLevel,
        toLevel,
...
router.post(
  "/users/:userId/cards/:cardId/top-up",
  asyncHandler(async (req, res) => {
    const user = await db.getUser(req.params.userId);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    ...
    if (amount < MIN_TOP_UP_AMOUNT) {
      return res
        .status(400)
        .json({ error: `Minimum top-up is $${MIN_TOP_UP_AMOUNT}.` });
    }

    if (amount > MAX_TOP_UP_AMOUNT) {
      return res
        .status(400)
        .json({ error: `Maximum top-up is $${MAX_TOP_UP_AMOUNT}.` });
    }

    if (
      (Number(card.multi_currency?.SGD ?? card.balance) || 0) + amount >
      MAX_WALLET_BALANCE
    ) {
      return res.status(400).json({
    ...
    if (sourceCardId || method === "linked") {
      if (!sourceCardId) {
        return res
          .status(400)
          .json({ error: "Select a linked bank card to pay from." });
      }
      ...
      if (!sourceCard || sourceCard.card_type !== "others") {
        return res
          .status(400)
          .json({ error: "Select a valid linked bank card." });
      }

      const normalized = cardUtils.normalizeCardRow(sourceCard);
      if (
        normalized.account_kind !== "debit" &&
        normalized.account_kind !== "credit"
      ) {
        return res
          .status(400)
          .json({ error: "Select a valid linked debit or credit card." });
      }

      if (
        (Number(sourceCard.multi_currency?.SGD ?? sourceCard.balance) || 0) -
          amount <
        SOURCE_CARD_RESERVE
      ) {
        return res.status(400).json({
    ...
    if (sourceCard) {
      ...
      if (sourceTransaction) {
        responsePayload.sourceTransaction =
          formatTransaction(sourceTransaction);
      }
    }
...
router.patch(
  "/users/:userId/transactions/:txnId/rewards",
  asyncHandler(async (req, res) => {
    const user = await db.getUser(req.params.userId);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    ...
    if (!Number.isFinite(xpGained) || xpGained < 0) {
      return res
        .status(400)
        .json({ error: "xpGained must be a non-negative number." });
    }

    const updated = await db.updateTransactionRewards(
      req.params.userId,
      req.params.txnId,
      {
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const transactionFilters = await resolveTransactionFilters(
      req.params.userId,
      req.query,
    );
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(
      buildInsights(transactions, selectedPeriod.month, selectedPeriod.year),
    );
  }),
);

router.get(
  "/users/:userId/dna-profile",
  asyncHandler(async (req, res) => {
    const user = await db.getUser(req.params.userId);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
...
async function resolveTransactionFilters(userId, query) {
  ...
  if (
    query.cardType &&
    ["prepaid", "cashcard", "others"].includes(String(query.cardType))
  ) {
...
function mapCard(row, view = "summary") {
  ...
  const base = {
    ...
    creditLimit: row.credit_limit
      ? clampCreditLimit(row.credit_limit)
      : isCredit
        ? clampCreditLimit(3000)
        : null,
  ...
  return {
    ...base,
    cardNumber: digits
      ? netsSimulator.formatCardNumber(digits)
      : row.masked_number,
...
router.get(
  "/users/:userId/cards/:cardId/wallet",
  asyncHandler(async (req, res) => {
    const user = await db.getUser(req.params.userId);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
...
/** Exchange currency */
router.post(
  "/users/:userId/cards/:cardId/exchange",
  asyncHandler(async (req, res) => {
    const user = await db.getUser(req.params.userId);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
...
router.post(
  "/users/:userId/cards/:cardId/deduct",
  asyncHandler(async (req, res) => {
    const user = await db.getUser(req.params.userId);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    ...
    if (!currency || amount === undefined || amount === null) {
      return res
        .status(400)
        .json({ error: "currency and amount are required." });
    }
    ...
    const hasMultiCurrency =
      cardData.multi_currency && typeof cardData.multi_currency === "object";

    let result;
    if (hasMultiCurrency) {
      result = await deductFromMultiCurrency(
        req.params.cardId,
        currency,
        deductAmount,
      );
    } else {
      const updatedCard = await updateCardBalance(
        req.params.cardId,
        -deductAmount,
      );
      result = updatedCard
        ? {
            ok: true,
            card: updatedCard,
            newBalances: { SGD: updatedCard.balance },
          }
        : { ok: false, error: "Deduction failed" };
    }
